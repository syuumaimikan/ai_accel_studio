...
def main():
    ...
    mb=sub.add_parser('model-bench'); mb.add_argument('--models',required=True,help='comma-separated HF IDs/paths'); mb.add_argument('--methods',default='baseline-fp16,custom-int2-accuracy-guard'); mb.add_argument('--prompt',default='Explain GPU memory bandwidth in one sentence.'); mb.add_argument('--calibration',default='Explain matrix multiplication.'); mb.add_argument('--eval-text',default='Efficient machine learning inference requires careful use of computation and memory.'); mb.add_argument('--new-tokens',type=int,default=32); mb.add_argument('--dtype',default='fp16',choices=['fp16','bf16']); mb.add_argument('--group-size',type=int,default=128)
    ...
    if args.cmd=='hardware':
        ...
        df=benchmark_matrix(models,methods,args.prompt,[args.calibration],args.eval_text,args.new_tokens,'cuda',args.dtype,group_size=args.group_size)
